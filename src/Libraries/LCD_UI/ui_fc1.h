// ui defs for FEATURE_CONTROLLER == 1
...
#else // UI_DISPLAY_TYPE != 3, Direct display connections
...
which must be added inside initKeys() and the second ist a testing routine.
These come into checkKeys() or checkSlowKeys() depending on the time needed
for testing. If you are in doubt, put it in checkSlowKeys().
initKeys() is called from an interrupt controlling the extruder, so only
...
As all I2C request take time, they belong all in checkSlowKeys().
...
inline void UIDisplay::checkSlowEncoder() {
#if defined(UI_HAS_I2C_KEYS) && UI_HAS_KEYS != 0
#if UI_DISPLAY_I2C_CHIPTYPE == 0
	i2cStartWait(UI_I2C_KEY_ADDRESS + I2C_READ);
	uint8_t keymask = i2cReadNak(); // Read current key mask
#endif
#if UI_DISPLAY_I2C_CHIPTYPE == 1
    i2cStartWait(UI_DISPLAY_I2C_ADDRESS +I 2C_WRITE);
    i2cWrite(0x12); // GIOA
    i2cStop();
...
#endif
	i2cStop();
  // Add I2C click encoder tests here, all other i2c tests and a copy of the encoder test belog in checkSlowKeys()
