...
static volatile SDCardState sdCardState;

#if defined(SD_DETECT_PIN) && defined(SD_DETECT_VAL)

void SdCardDetectInterrupt()
{
	sdCardState = JUST_REMOVED;
	if (reprap.GetPlatform()) reprap.GetPlatform()->InvalidateFiles();
}
...
bool MassStorage::FileSystemAvailable()
{
	if (sdCardState == READY) {
		return true;
	}

	// SD card has been removed at some point

	// See if we need to clear state
	if (sdCardState == JUST_REMOVED)
	{
		Reset();
	}

	// Now see if the card has been re-inserted
	if (digitalRead(SD_DETECT_PIN) == SD_DETECT_VAL)
	{
		if (fileSystemInit()) {
			sdCardState = READY;
			return true;
...
bool MassStorage::fileSystemInit()
{
	sd_mmc_init();
	...
	return (FR_OK == f_mount(0, &fileSystem));
}
...
bool MassStorage::Delete(const char* directory, const char* fileName)
{
	...
	const char* location = (directory != NULL) ? CombineName(directory, fileName) : fileName;
...
{
	if (!platform->GetMassStorage()->FileSystemAvailable()) return false;
