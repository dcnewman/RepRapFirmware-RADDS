...
bool FileStore::Open(const char* directory, const char* fileName, bool write)
{
	...
	if (!platform->GetMassStorage()->FileSystemAvailable())
	{
		if (reprap.Debug(modulePlatform))
		{
			platform->MessageF(GENERIC_MESSAGE, "Can't open %s to %s, file system unavailable\n", location, (writing) ? "write" : "read");
		}
		return false;
	}
...
FilePosition FileStore::Position() const
{
	if (!inUse)
	{
		// File may have been closed by removal of the SD card
		platform->Message(GENERIC_MESSAGE, "Attempt to get position in a non-open file.\n");
