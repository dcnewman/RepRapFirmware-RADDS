...
#if defined(LCD_UI)
float Tool::MaxAcceleration() const
{
	if (driveCount == 0)
	{
		reprap.GetPlatform()->Message(GENERIC_MESSAGE, "Attempt to get maximum feedrate for a tool with no drives.\n");
		return 1.0;
	}
	float result = 0.0;
	for (size_t d = 0; d < driveCount; d++)
	{
		float mf = reprap.GetPlatform()->Acceleration(drives[d] + AXES);
		if (mf > result)
		{
			result = mf;
		}
