...
GCodes::GCodes(Platform* p) :
		platform(p), active(false), stackPointer(0), auxGCodeReply(nullptr), isFlashing(false)
#endif
{
#if defined(WEBSERVER)
...
#if defined(LCD_UI)
	lcdGCode = new GCodeBuffer(platform, "lcd: ");
#endif
}
...
#if defined(LCD_UI)
	// LCD UI
	// Process commands from the LCD UI.  Typically these are either
	// commands to print a SD file, jog, or load or unload filament.
	if (!lcdGCode->Active() && lcdUiInput->GCodeAvailable())
	{
		int8_t i = 0;
		do
		{
			char b = lcdUiInput->ReadGCode();
			if (lcdGCode->Put(b))
			{
				lcdGCode->SetFinished(ActOnCode(lcdGCode, reply));
			}
			break;	// stop after receiving a complete gcode in case we haven't finished processing it
			++i;
		} while (i < 16 && lcdUiInput->GCodeAvailable());
		platform->ClassReport(longWait);
		return;
	}
#endif
...
void GCodes::HandleReply(GCodeBuffer *gb, bool error, const char* reply)
{
	...
	const Compatibility c = (gb == serialGCode
#if defined(WEBSERVER)
							 || gb == telnetGCode
#endif
		) ? platform->Emulating() : me;
...
void GCodes::SetHeaterParameters(GCodeBuffer *gb, StringRef& reply)
{
	if (gb->Seen('P'))
	{
		...
		if (heater >= 0 && heater < HEATERS)
		{
			...
			if (gb->Seen('X'))
			{
				// Valid thermistor numbers are in the inclusive ranges [0, HEATERS-1] and [100, 100 + (MAX31855_DEVICES - 1)]
...
bool GCodes::HandleMcode(GCodeBuffer* gb, StringRef& reply)
{
	...
	case 20:		// List files on SD card
	{
		...
		else
		{
			...
			bool encapsulateList = ((gb != serialGCode
#if defined(WEBSERVER)
									 && gb != telnetGCode
#endif
										) || platform->Emulating() != marlin);
			FileInfo fileInfo;
	...
	case 220:	// Set/report speed factor override percentage
		if (gb->Seen('S'))
		{
			if (!(SetSpeedFactor(gb->GetFValue() / 100.0)))
			{
		...
		{
			reply.printf("Speed factor override: %.1f%%", GetSpeedFactor() * 100.0);
		}
		break;
	...
	case 221:	// Set/report extrusion factor override percentage
		{
			...
			{
				SetExtrusionFactor(extruder, gb->GetFValue());
			}
			else
			{
	...
	case 569: // Set/report axis direction and enable values
		if (gb->Seen('P'))
		{
...
bool GCodes::SetExtrusionFactor(int extruder, float extrusionFactor)
{
	if ((extruder < 0) || ((size_t)extruder >= (DRIVES - AXES)))
		return false;

	extrusionFactor = extrusionFactor / 100.0;
...
bool GCodes::SetSpeedFactor(float newSpeedFactor)
{
	newSpeedFactor = newSpeedFactor * secondsToMinutes;// include the conversion from mm/minute to mm/second
