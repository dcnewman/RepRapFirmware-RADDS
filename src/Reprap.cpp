...
#if defined(LCD_UI)
	// gCodes needed in order to pass UIDisplay's gcode input buffer to GCodes::
	lcdui = new UIDisplay(platform, heat, gCodes);
#endif
...
void RepRap::Init()
{
	...
	printMonitor->Init();
#if defined(LCD_UI)
	// This initializes comms with the LCD... potentially on the slow side
	lcdui->Init();
#endif
...
OutputBuffer *RepRap::GetLegacyStatusResponse(uint8_t type, int seq)
{
	...
	if (type < 2)
	{
		response->catf(",\"buff\":%u",
#if defined(WEBSERVER)
					   webserver->GetGCodeBufferSpace(WebSource::HTTP));	// send the amount of buffer space available for gcodes
#else
		0);
#endif
	}
